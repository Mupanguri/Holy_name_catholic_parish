...
// Hardcoded fallback data — used when no CMS content exists yet
    ...
    subFields: [
      {
        ...
        numbers: [{ number: '+263 242 306345', email: 'holynamemabelreign@gmail.com' }],
      },
      {
        subField: 'Parish Priest',
        numbers: [{ name: 'Fr Ndhlalambi', number: '+263772402220', email: 'jndhlalambi@gmail.com' }],
      },
      {
        subField: 'Assistant Parish Priest',
        numbers: [{ name: 'Fr Jingisoni', number: '+263772300024', email: 'giftjingison@gmail.com' }],
      },
    ],
  },
  {
    ...
    subFields: [
      {
        subField: 'Emily Mari',
        numbers: [{ name: 'Emily Mari', number: '+263777163212' }],
      },
      {
        subField: 'Sacred Heart Youth Guild',
        numbers: [{ name: 'Chairperson: Arthur Hukama', number: '+263 783808377' }],
      },
      {
        subField: 'Hosi Yedenga',
        numbers: [
          { name: 'Mrs M Choto', number: '+263772847374' },
          { name: 'Mrs P Mautsa', number: '+263733756454' },
        ],
      },
    ],
  },
    ...
    subFields: [
      {
        ...
        numbers: [{ name: 'Mr V Mudimu', number: '+263773382139' }],
      },
      {
        subField: 'Bloomingdale',
        numbers: [{ name: 'Mr Chisuro', number: '+263781907444' }],
      },
      {
        subField: 'Meyrick Park',
        numbers: [{ name: 'Mrs Thoko Nyandoro', number: '+263772423383' }],
      },
    ],
  },
  {
    ...
    subFields: [
      {
        subField: 'SOCCOM',
        numbers: [
          { name: 'Mr D Kunaka', number: '+263 775063153' },
          { name: 'Hunter Mupfurutsa', number: '+263 789864886' },
        ],
      },
      {
        subField: 'Catechesis',
        numbers: [{ name: 'Mr Felix Manyimbiri', number: '+263772392965' }],
      },
      {
      ...
      },
      {
        ...
        numbers: [{ name: 'Ms Leonora Mawire', number: '+263 774161458' }],
      },
    ],
  },
];
...
const ContactPage = () => {
  const { getPageBySlug, pages } = useAuth();

  // Try to load CMS content for the contact page
  const cmsPage = getPageBySlug('contact');

  // Pages are hydrated but contact page is absent → it's invisible or in draft
  if (!cmsPage && pages && pages.length > 0) {
    return <ComingSoon title="Contact Us" message="This page is temporarily unavailable." />;
  }

  let contacts = DEFAULT_CONTACTS;
  let pageTitle = 'Contact Us';
  let pageSub = 'Get in touch with Holy Name Parish';

  if (cmsPage?.content) {
    try {
      const parsed = JSON.parse(cmsPage.content);
      // Support both { contacts, title, subtitle } wrapper and raw array
      if (Array.isArray(parsed)) {
        contacts = parsed;
      } else {
        if (Array.isArray(parsed.contacts)) contacts = parsed.contacts;
        if (parsed.title) pageTitle = parsed.title;
        if (parsed.subtitle) pageSub = parsed.subtitle;
      }
    } catch {
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
          <div className="hn-parchment-container">
            ...
            <div className="relative p-6">
              <h1 className="hn-section-heading">{pageTitle}</h1>
              <p className="hn-section-sub">{pageSub}</p>
