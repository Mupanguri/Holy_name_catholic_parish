...
function getCurrentSeason(): keyof typeof SEASONAL_PALETTES {
...
const BRANCH_CONFIG = [
  // LEFT SIDE (angles > 90 degrees, curving left)
  { 
    id: "sections", 
    label: "Sections", 
    side: "left",
  ...
  // RIGHT SIDE (angles < 90 degrees, curving right)
  { 
    id: "support-teams", 
    label: "Support Teams", 
    side: "right",
...
function ArcBranch({
  ...
  return (
    ...
    >
      ...
      {/* Main thick branch shape using path outline */}
      <path
        d={`
...
// Sub-branch component (smaller branches coming off main)
function SubBranch({
  startX,
  startY,
  angle,
...
function BranchInfoCard({
  ...
  branch: (typeof BRANCH_CONFIG)[0]
  pages: Page[]
  onClose: () => void
  onPageClick?: (page: Page) => void
  palette: (typeof SEASONAL_PALETTES)["spring"]
}) {
  ...
  return (
    <div 
      className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-80 max-w-[90vw] animate-in fade-in zoom-in-95 duration-200"
                    ...
                    {mainPages.map(page => (
                      <li key={page.id}>
                        <button
                          ...
                          className="w-full text-left px-3 py-2 rounded-lg hover:bg-muted transition-colors flex items-center gap-2 group"
                        >
                          <span 
...
export function OakTree({
  ...
  const selectedBranchConfig = selectedBranch 
    ? BRANCH_CONFIG.find(b => b.id === selectedBranch) 
    : null

  // Calculate point along bezier for sub-branches
    ...
    <div className={cn("relative w-full max-w-3xl mx-auto", className)}>
      ...
      <div className="absolute top-4 left-4 z-10">
        <span
          ...
          style={{ backgroundColor: palette.canopyDark }}
        >
      ...
      <div className="absolute top-4 right-4 z-10 text-right space-y-1">
        <div className="text-sm text-muted-foreground">
          <span className="font-bold text-foreground">{mainPages.length}</span> branches
        </div>
        <div className="text-sm text-muted-foreground">
          <span className="font-bold text-foreground">{subPages.length}</span> leaves
        </div>
      </div>
      ...
      <svg viewBox="0 0 400 600" className="w-full h-auto" style={{ minHeight: 550 }}>
        ...
        {/* SOIL AND GROUND SECTION */}
        {/* Rocky sandy ground base */}
        ...
        {/* Grass/moss at surface */}
        <ellipse cx="200" cy="512" rx="100" ry="8" fill="#5D6B3D" opacity="0.4" />

        {/* ROOT SYSTEM */}
        {/* Main Tap Root - thick and goes deep, fading */}
        <path
          d={`
            M200,515
            Q200,545 198,570
        ...
        {/* Secondary roots spreading out and fading */}
        {/* Left side roots */}
        <path
          d="M185,525 Q140,540 80,560 Q40,575 10,600"
          fill="none"
          stroke="url(#sideRootGrad)"
          strokeWidth="12"
          strokeLinecap="round"
        />
        <path
          d="M175,535 Q130,555 90,575 Q60,590 40,600"
          fill="none"
          stroke="url(#sideRootGrad)"
          strokeWidth="8"
          strokeLinecap="round"
        />
        <path
          d="M180,545 Q150,560 120,580 Q100,592 85,600"
          fill="none"
          stroke="url(#sideRootGrad)"
          strokeWidth="6"
          strokeLinecap="round"
        ...
        {/* Right side roots */}
        <path
          d="M215,525 Q260,540 320,560 Q360,575 390,600"
          fill="none"
          stroke="url(#sideRootGrad)"
          strokeWidth="12"
          strokeLinecap="round"
        />
        <path
          d="M225,535 Q270,555 310,575 Q340,590 360,600"
          fill="none"
          stroke="url(#sideRootGrad)"
          strokeWidth="8"
          strokeLinecap="round"
        />
        <path
          d="M220,545 Q250,560 280,580 Q300,592 315,600"
          fill="none"
          stroke="url(#sideRootGrad)"
          strokeWidth="6"
          strokeLinecap="round"
        ...
        {/* Exposed surface roots connecting to trunk */}
        ...
        {BRANCH_CONFIG.map((branchConfig, idx) => {
          ...
          return (
            <g 
              key={branchConfig.id}
              onMouseEnter={() => setHoveredBranch(branchConfig.id)}
              onMouseLeave={() => setHoveredBranch(null)}
            >
              {/* Main colored arc branch */}
              <ArcBranch
              ...
              {/* Sub-branches for each page */}
              {mainBranchPages.map((page, pageIdx) => {
                const t = 0.3 + (pageIdx * 0.2)
                const point = getBezierPoint(
                  t, 
                  branchConfig.arcStart, 
                  branchConfig.arcControl1, 
                  branchConfig.arcControl2, 
                  branchConfig.arcEnd
                )
        ...
        {/* Canopy layers - expanded to fully cover branch tips */}
        {/* Base large canopy to cover all branches */}
        ...
        {/* Edge extensions to cover branch tips completely */}
        ...
        {/* LABEL SECTION WITH TRACE LINES */}
        {/* Labels at trunk base level, at card margins, with hair-thin trace lines */}
        {BRANCH_CONFIG.map((branchConfig) => {
          const isLeft = branchConfig.side === "left"
          // Get a point on the branch to trace to (slightly above start of branch)
          const branchPoint = {
          ...
          return (
            <g key={`label-${branchConfig.id}`}>
              {/* Hair-thin trace line from label to branch - very faint like thread */}
              <path
                d={`
                  M${branchConfig.labelPos.x + (isLeft ? 50 : -50)},${branchConfig.labelPos.y - 4}
                  Q${isLeft ? branchConfig.labelPos.x + 80 : branchConfig.labelPos.x - 80},${(branchConfig.labelPos.y + branchPoint.y) / 2}
              ...
              {/* Tiny dot at branch connection - barely visible */}
              <circle
                cx={branchPoint.x}
                cy={branchPoint.y}
                r="2"
              ...
              {/* Small colored dot before/after label */}
              <circle
