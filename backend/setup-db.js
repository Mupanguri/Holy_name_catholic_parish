...
const createTables = async () => {
    ...
    // Note: Using separate queries instead of DO blocks to avoid template literal issues
    const alterTablesSQL = [];
    
    // Add parent_id to pages if not exists
    alterTablesSQL.push(`
        DO $
        BEGIN
    ...
    // Add change_description to submissions if not exists
    alterTablesSQL.push(`
        DO $
        BEGIN
            IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'submissions' AND column_name = 'change_description') THEN
                ALTER TABLE submissions ADD COLUMN change_description TEXT;
            END IF;
        END $;
    `);
        ...
        for (const sql of alterTablesSQL) {
            try {
                ...
                if (!err.message.includes('already exists')) {
                    console.log("⚠️  Alter table note:", err.message);
                }
            }
        }
